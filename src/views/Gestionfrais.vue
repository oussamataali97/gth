            ...
            <table class="content-table">
              ...
              <tbody>
                <tr v-for="(index, i) in filterFrais" :key="index._id">
                  ...
                  <td>{{ index.prenom }}</td>
                  <td>
                    <li @click="generateWord(index._id, index.nom, index.prenom)"><i class="fa-solid fa-file-word"></i>
                    </li>
                    <li @click="generatexslx(index._id)"><i class="fa-solid fa-file-excel"></i></li>
                  </td>
                  <td>{{ index.totalGeneral }} DH</td>
                  <td>
                    <button type="submit" :class="index.valideRH ? 'succesButton' : 'echecButton'"
                      @click="validerh(index._id)">Validation par DT - RH </button>
                    <button type="submit" v-if="index.valideRH"
                      :class="index.valideDIR ? 'succesButton' : 'echecButton'"
                      @click="validedir(index._id, i)">Validation Administration</button>
                  </td>
                  <td>
                    ...
                    <input type="text" v-model="frais[i].refTransaction" v-if="index.valideDIR == true"
                      :disabled="index.etatPaiment">
                    <button class="confirmer" v-if="index.valideDIR == true && index.etatPaiment == false" type="button" value="valider"
                      @click="paiment(index._id, i)"><i class="fa-solid fa-circle-check"></i></button>

                      <button class="supprimer" v-if="index.etatPaiment == true" type="button" value="supprimer "
                      @click="supprimerPaiment(index._id)"><i class="fa-solid fa-trash"></i></button>
                  </td>
                  <td>
                    <input type="date" v-model="frais[i].datePaiment"
                      v-if="index.valideDIR == true && index.validerDatePaiment == false"
                      :disabled="index.validerDatePaiment">
                    <span v-if="index.validerDatePaiment == true"> {{ new
                        Date(frais[i].datePaiment).toLocaleDateString()
                    }}</span>
                    <button class="confirmer" v-if="index.valideDIR == true && index.validerDatePaiment == false" type="button"
                      value="confirmer" @click="paimentDate(index._id, i)"><i
                        class="fa-solid fa-circle-check"></i></button>

                  </td>
                  <td>
                    <button type="submit"
                      @click="supprimerFrais(index._id, index.mois, index.annee, index.matricule, i)"><i class="fa-solid fa-trash-can"></i></button>
                  </td>
                  <td>
                    <li v-if="index.notification" @click="DeleteCheckNotificationFrais(index._id, i)"><i
                        class="fa-solid fa-eye action"></i></li>
                  </td>
                </tr>
              </tbody>
            </table>
